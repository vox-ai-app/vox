...
function syncTokensToCss(tokens) {
  const style = document.documentElement.style
  for (const [key, value] of Object.entries(tokens)) {
    const kebab = camelToKebab(key)
    style.setProperty(`--vox-${kebab}`, value)
    style.setProperty(`--${kebab}`, value)
  }
  style.setProperty('--brand-start', tokens.accent)
  style.setProperty('--brand-ink', tokens.textOnAccent)
}

let activeTokens = darkColors
syncTokensToCss(activeTokens)
