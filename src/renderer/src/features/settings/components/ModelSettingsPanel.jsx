...
const SUGGESTED = [
  { label: 'Qwen 2.5 3B (fast, ~2 GB)', hfRepo: 'Qwen/Qwen2.5-3B-Instruct-GGUF',             hfFile: 'qwen2.5-3b-instruct-q4_k_m.gguf' },
  { label: 'Llama 3.2 3B (~2 GB)',       hfRepo: 'bartowski/Llama-3.2-3B-Instruct-GGUF',      hfFile: 'Llama-3.2-3B-Instruct-Q4_K_M.gguf' },
  { label: 'Mistral 7B (~4 GB)',          hfRepo: 'TheBloke/Mistral-7B-Instruct-v0.2-GGUF',   hfFile: 'mistral-7b-instruct-v0.2.Q4_K_M.gguf' }
]
...
export default function ModelSettingsPanel() {
  ...
  useEffect(() => {
    ...
    window.api.models.getDownloads?.().then((active) => {
  ...
  const handleDownload = async ({ hfRepo, hfFile }) => {
    ...
    try {
      ...
      setDownloads((prev) => { const n = { ...prev }; delete n[hfFile]; return n })
  ...
  return (
    <article className="workspace-panel-card">
      ...
      {feedback && (
        <p className={`knowledge-rail-feedback knowledge-rail-feedback-${feedback.type}`} style={{ marginBottom: '10px' }}>
          {feedback.text}
        </p>
      )}
            ...
            return (
              ...
              >
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontSize: '0.86rem', fontWeight: 500, color: 'var(--vox-text-primary)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                    {m.filename}
                  </div>
                  <div style={{ fontSize: '0.72rem', color: 'var(--vox-text-muted)', marginTop: '2px' }}>
                    {formatBytes(m.size)}{m.hfRepo ? ` · ${m.hfRepo}` : ''}
                  </div>
                </div>
                <div style={{ display: 'flex', gap: '6px', flexShrink: 0, alignItems: 'center' }}>
                  {isActive
                    ? <span style={{ fontSize: '0.72rem', color: 'rgba(236,137,184,0.9)', fontWeight: 600, padding: '2px 8px' }}>Active</span>
                    : <button className="chat-task-card-btn" onClick={() => handleSetActive(m.path)} type="button">Use</button>
                  }
                  <button
      ...
      {models.length === 0 && Object.keys(downloads).length === 0 && (
        <p style={{ fontSize: '0.86rem', color: 'var(--vox-text-secondary)', marginBottom: '14px' }}>
      ...
      <div style={{ display: 'grid', gap: '6px' }}>
        ...
        <details open={models.length === 0} style={{ marginTop: '4px' }}>
          <summary style={{ fontSize: '0.8rem', color: 'var(--vox-text-secondary)', cursor: 'pointer', userSelect: 'none', padding: '4px 0' }}>
                ...
                <div key={s.hfFile}>
                  ...
                  >
                    {exists
                      ...
                      : inProgress
                        ? <span style={{ width: '12px', height: '12px', borderRadius: '50%', border: '2px solid rgba(236,137,184,0.2)', borderTopColor: '#ec89b8', display: 'inline-block', animation: 'workspaceButtonSpin 0.8s linear infinite', flexShrink: 0 }} />
                        : null
                    }
                    <span style={{ flex: 1, textAlign: 'left' }}>
                      {inProgress ? `Downloading… ${dl.percent > 0 ? `${dl.percent}%` : ''}` : s.label}
                    </span>
                  </button>
                  {inProgress && dl.percent > 0 && (
                    <div style={{ height: '3px', borderRadius: '999px', background: 'var(--vox-border-soft)', overflow: 'hidden', marginTop: '4px' }}>
                      <div style={{ height: '100%', borderRadius: '999px', background: 'rgba(236,137,184,0.7)', width: `${dl.percent}%`, transition: 'width 0.3s ease' }} />
                    </div>
                  )}
                </div>
              )
